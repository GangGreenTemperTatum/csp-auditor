...
export class EnhancedCspAnalyzer {
  static analyzePolicy(
    ...
    vulnerabilities.push(
      ...this.analyzeCriticalVulnerabilities(policy, settings),
    );
   ...
   */
  private static analyzeCriticalVulnerabilities(
    policy: CspPolicy,
    settings?: Record<string, boolean>,
  ): CspVulnerability[] {
    const vulnerabilities: CspVulnerability[] = [];
    ...
    for (const [, directive] of policy.directives) {
      ...
      if (this.isScriptExecutionDirective(directive.name)) {
        for (const value of directive.values) {
          ...
          if (
            value === "*" &&
            this.isCheckEnabled("script-wildcard", settings)
          ) {
            vulnerabilities.push(
          ...
          if (
            value === "'unsafe-inline'" &&
            this.isCheckEnabled("script-unsafe-inline", settings)
          ) {
            vulnerabilities.push(
              this.createVulnerability({
          ...
          if (
            value === "'unsafe-eval'" &&
            this.isCheckEnabled("script-unsafe-eval", settings)
          ) {
            vulnerabilities.push(
              this.createVulnerability({
          ...
          if (value === "data:") {
            vulnerabilities.push(
              this.createVulnerability({
                ...
                description:
                  "Allows base64-encoded JavaScript execution via data: URIs",
                remediation:
   ...
   */
  private static analyzeBypassTechniques(
    policy: CspPolicy,
    settings?: Record<string, boolean>,
  ): CspVulnerability[] {
    const vulnerabilities: CspVulnerability[] = [];
   ...
   */
  private static analyzeLevel3Features(
    policy: CspPolicy,
    settings?: Record<string, boolean>,
  ): CspVulnerability[] {
    const vulnerabilities: CspVulnerability[] = [];
   ...
   */
  private static analyzePolicyWeaknesses(
    policy: CspPolicy,
    settings?: Record<string, boolean>,
  ): CspVulnerability[] {
    const vulnerabilities: CspVulnerability[] = [];
   ...
   */
  private static analyzeModernThreats(
    policy: CspPolicy,
    settings?: Record<string, boolean>,
  ): CspVulnerability[] {
    const vulnerabilities: CspVulnerability[] = [];
  ...
  private static checkJsonpBypasses(
    policy: CspPolicy,
        ...
        if (value.includes(riskyHost)) {
          vulnerabilities.push(
            this.createVulnerability({
              ...
              remediation:
                "Remove JSONP-enabled hosts or use fetch() with proper CORS",
              requestId: policy.requestId,
            }),
          );
        }
  ...
  private static checkAngularJsBypasses(
    policy: CspPolicy,
    vulnerabilities: CspVulnerability[],
  ): void {
    const scriptSrc = policy.directives.get("script-src");
      ...
      if (value.includes("angular") && !value.includes("angular.min.js")) {
        vulnerabilities.push(
          this.createVulnerability({
            ...
            description:
              "AngularJS versions allow template injection bypasses of CSP",
  ...
  private static analyzeNonceHashSecurity(
    policy: CspPolicy,
    vulnerabilities: CspVulnerability[],
  ): void {
    const scriptSrc = policy.directives.get("script-src");
  ...
  private static checkServiceWorkerBypasses(
    _policy: CspPolicy,
    _vulnerabilities: CspVulnerability[],
  ): void {
    // Service worker bypass analysis
  }

  private static checkCssInjectionRisks(
    _policy: CspPolicy,
    _vulnerabilities: CspVulnerability[],
  ): void {
  ...
  private static checkWasmThreats(
    _policy: CspPolicy,
    _vulnerabilities: CspVulnerability[],
  ): void {
  ...
  private static analyzeDeprecatedFeatures(
    policy: CspPolicy,
    settings?: Record<string, boolean>,
  ): CspVulnerability[] {
    const vulnerabilities: CspVulnerability[] = [];
    ...
    if (
      ...
      ].includes(policy.headerName.toLowerCase())
    ) {
      vulnerabilities.push(
        this.createVulnerability({
