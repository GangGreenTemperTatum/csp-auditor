...
export const MODERN_SUPPLY_CHAIN_RISKS = [
  ...
  { domain: "polyfill.io", risk: "Compromised CDN serving malicious polyfills", severity: "high" },
  { domain: "bootcdn.cn", risk: "Chinese CDN with potential state-level risks", severity: "high" },
  { domain: "staticfile.org", risk: "Unverified Chinese CDN", severity: "medium" },
  
  // Package manager CDNs with supply chain risks
  { domain: "unpkg.com", risk: "NPM package CDN - supply chain attack vector", severity: "medium" },
  { domain: "cdn.skypack.dev", risk: "ES module CDN with potential risks", severity: "medium" },
  { domain: "jspm.dev", risk: "Module CDN with limited security guarantees", severity: "medium" },
];

// AI/ML service endpoints (potential data exfiltration)
export const AI_ML_SERVICE_RISKS = [
  { domain: "api.openai.com", risk: "AI API - potential data exfiltration", severity: "medium" },
  { domain: "api.anthropic.com", risk: "AI API - potential sensitive data exposure", severity: "medium" },
  { domain: "huggingface.co", risk: "ML model hosting - code execution risks", severity: "medium" },
  { domain: "replicate.com", risk: "ML API service - data privacy concerns", severity: "medium" },
  { domain: "colab.research.google.com", risk: "Jupyter notebook execution environment", severity: "high" },
];

// Cryptocurrency/Web3 integration risks
export const WEB3_INTEGRATION_RISKS = [
  { domain: "metamask.io", risk: "Wallet integration - financial transaction risks", severity: "high" },
  { domain: "walletconnect.org", risk: "Cross-wallet protocol - authentication bypass", severity: "high" },
  { domain: "uniswap.org", risk: "DeFi protocol - financial manipulation", severity: "high" },
  { domain: "pancakeswap.finance", risk: "DeFi exchange - smart contract risks", severity: "high" },
  { domain: "web3.storage", risk: "Decentralized storage - content integrity issues", severity: "medium" },
];

// Social media embed risks (privacy and tracking)
export const SOCIAL_EMBED_RISKS = [
  { domain: "platform.twitter.com", risk: "Twitter embed tracking and XSS risks", severity: "medium" },
  { domain: "connect.facebook.net", risk: "Facebook tracking and data collection", severity: "medium" },
  { domain: "www.instagram.com", risk: "Instagram embed privacy concerns", severity: "low" },
  { domain: "platform.linkedin.com", risk: "LinkedIn tracking integration", severity: "medium" },
  { domain: "assets.pinterest.com", risk: "Pinterest tracking and analytics", severity: "low" },
];

// Modern analytics and tracking (2024 landscape)
export const MODERN_TRACKING_RISKS = [
  { domain: "googletagmanager.com", risk: "Comprehensive user tracking and analytics", severity: "medium" },
  { domain: "hotjar.com", risk: "Session recording and user behavior tracking", severity: "high" },
  { domain: "fullstory.com", risk: "Complete session recording including sensitive data", severity: "high" },
  { domain: "logrocket.com", risk: "Application monitoring with PII exposure", severity: "high" },
  { domain: "sentry.io", risk: "Error tracking that may capture sensitive data", severity: "medium" },
];

// Gaming and metaverse platforms
export const GAMING_METAVERSE_RISKS = [
  { domain: "unity3d.com", risk: "Unity WebGL player - arbitrary code execution", severity: "high" },
  { domain: "unrealengine.com", risk: "Unreal Engine web player risks", severity: "high" },
  { domain: "roblox.com", risk: "User-generated content platform", severity: "medium" },
  { domain: "minecraft.net", risk: "Gaming platform with user content", severity: "medium" },
];

// Enhanced user content hosts (2024 update)
export const ENHANCED_USER_CONTENT_HOSTS = [
  // All previous hosts plus modern platforms
  ...["*.github.io", "raw.githubusercontent.com", "*.s3.amazonaws.com", "*.herokuapp.com"],
  
  // Modern development platforms
  "replit.com", "*.repl.co", "codesandbox.io", "*.csb.app",
  "stackblitz.com", "*.stackblitz.io", "glitch.com", "*.glitch.me",
  
  // Modern hosting platforms
  "vercel.app", "*.vercel.app", "netlify.app", "*.netlify.app",
  "render.com", "*.onrender.com", "railway.app", "*.railway.app",
  
  // No-code platforms
  "webflow.io", "*.webflow.io", "bubble.io", "*.bubble.io",
  "notion.so", "*.notion.so", "airtable.com", "*.airtable.com",
];
...
export class EnhancedBlacklistManager {
   ...
   */
  static checkSupplyChainRisk(domain: string): { isRisky: boolean; risk?: string; severity?: string } {
   ...
   */
  static checkAiMlServiceRisk(domain: string): { isRisky: boolean; risk?: string; severity?: string } {
    const cleanDomain = this.cleanDomain(domain);
   ...
   */
  static checkWeb3Risk(domain: string): { isRisky: boolean; risk?: string; severity?: string } {
    const cleanDomain = this.cleanDomain(domain);
   ...
   */
  static checkEnhancedVulnerableJs(domain: string, path?: string): {
    ...
    for (const vulnJs of ENHANCED_VULNERABLE_JS) {
      if (cleanDomain.includes(vulnJs.domain)) {
        ...
        if (vulnJs.paths && path) {
          const pathMatch = vulnJs.paths.some(vulnPath => path.includes(vulnPath));
          if (pathMatch) {
            return {
              isVulnerable: true,
              risk: vulnJs.risk,
   ...
   */
  static analyzeModernThreats(domain: string, path?: string): Array<{
    type: string;
    severity: string;
    risk: string;
    cve?: string[];
  }> {
    const threats: Array<{ type: string; severity: string; risk: string; cve?: string[] }> = [];
   ...
   */
  static checkPrivacyTrackingRisk(domain: string): { isTracking: boolean; risk?: string; severity?: string } {
    const cleanDomain = this.cleanDomain(domain);
  ...
  static getThreatIntelligenceSummary(): {
    ...
    return {
      ...
      trackingServices: [...SOCIAL_EMBED_RISKS, ...MODERN_TRACKING_RISKS].length
    };
  }
}