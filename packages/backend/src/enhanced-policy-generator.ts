...
export class EnhancedPolicyGenerator {
   ...
   */
  static generateSecurePolicy(options: {
  ...
  static generatePolicyFromAnalysis(analyses: CspAnalysisResult[]): {
    ...
    reasoning.push(`Analyzed ${analyses.length} CSP policies with ${allVulns.length} vulnerabilities`);

    // Determine security level needed
    const hasHighSeverity = allVulns.some(v => v.severity === 'high');
    const hasScriptVulns = vulnTypes.has('script-wildcard') || 
    ...
    if (hasHighSeverity || hasScriptVulns) {
      ...
      reasoning.push("High-severity vulnerabilities detected - recommending strict policy");
    }
    ...
    if (vulnTypes.has('script-wildcard')) {
      securityImprovements.push("Remove script-src wildcards and specify exact domains");
    }
    
    if (vulnTypes.has('script-unsafe-inline')) {
      securityImprovements.push("Replace 'unsafe-inline' with nonces or hashes");
    }
   ...
   */
  static generatePolicyForAppType(appType: 'spa' | 'static' | 'ecommerce' | 'media' | 'enterprise'): {
    policy: string;
    description: string;
    ...
    switch (appType) {
      case 'spa':
        ...
        description = "Strict CSP for Single Page Applications with modern security features";
        recommendations.push("Implement nonce generation for dynamic script loading");
        recommendations.push("Use Trusted Types to prevent DOM XSS");
        recommendations.push("Consider using 'strict-dynamic' for third-party script dependencies");
        break;
      ...
      case 'static':
        ...
        description = "Ultra-strict CSP for static websites with minimal attack surface";
        recommendations.push("Remove 'unsafe-inline' from style-src when possible");
      ...
      case 'ecommerce':
        ...
        description = "Balanced CSP for e-commerce with payment processor integration";
        recommendations.push("Whitelist only necessary payment processor domains");
        recommendations.push("Use subresource integrity (SRI) for payment scripts");
      ...
      case 'media':
        policy = this.generateSecurePolicy({
          ...
          allowInlineScripts: false
        }).replace("media-src 'self' data:", "media-src 'self' data: https: blob:");
        description = "Media-optimized CSP allowing various content sources";
        recommendations.push("Be cautious with blob: URLs as they can be exploited");
        recommendations.push("Consider using specific CDN domains instead of https:");
        break;
      ...
      case 'enterprise':
        ...
        recommendations.push("Implement CSP reporting endpoint for violation monitoring");
        recommendations.push("Use CSP report-to directive for modern browsers");
        recommendations.push("Regularly audit and update CSP based on violation reports");
        recommendations.push("Train development team on CSP-compliant coding practices");
        break;
   ...
   */
  static generateMigrationStrategy(currentAnalysis: CspAnalysisResult, targetPolicy: string): {
    steps: Array<{ step: number; description: string; policy: string; risk: 'low' | 'medium' | 'high' }>;
    timeline: string;
    riskAssessment: string;
  } {
    const steps: Array<{ step: number; description: string; policy: string; risk: 'low' | 'medium' | 'high' }> = [];
    
    // Phase 1: Deploy in report-only mode
    steps.push({
      step: 1,
      description: "Deploy recommended policy in Content-Security-Policy-Report-Only mode",
    ...
    steps.push({
      step: 2,
      description: "Fix critical violations reported in phase 1 (typically 1-2 weeks)",
    ...
    steps.push({
      ...
      policy: targetPolicy.replace("'strict-dynamic'", "'unsafe-inline'").replace("'none'", "'self'"),
      risk: 'medium'
    });
    ...
    const timeline = "Recommended timeline: 4-8 weeks depending on application complexity";
    const riskAssessment = "Progressive deployment minimizes risk of breaking application functionality";
  ...
  static scorePolicyStrength(policy: string): {
    ...
    if (!directiveMap.has('trusted-types')) {
      recommendations.push("Enable Trusted Types for enhanced DOM XSS protection");
    }
